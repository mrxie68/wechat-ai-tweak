...
- 设置页现在包含六样：**机器人开关**、**API Key**（掩码显示，不暴露明文）、**模型名**（点一下可选 `deepseek-v4-flash` / `deepseek-v4-pro` / 自定义）、**思考延迟（秒）**、**系统提示词**、**聊天风格样本**。点“保存”立即生效，存在微信本地，重启不丢；点“恢复默认”回到 [AIConfig.h](AIConfig.h) 里编译时的默认值。
