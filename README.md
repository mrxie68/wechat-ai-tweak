...
- 群聊默认关闭（防误回复）；在群聊信息页打开“AI 助手”开关后，群聊自动回复**所有消息，不需要任何 @**。
- **没有聊天命令**：聊天里不识别、也不需要任何 `AI xxx / @AI xxx` 命令，你的消息原样收发，一切设置都在设置页完成。
- **每个会话单独开关**：在目标会话（单聊或群聊）的聊天信息页里找到“AI 助手”行拨动开关即可（单聊默认开，群聊默认关）。
...
- 设置页从上到下：**机器人开关**、**AI 状态**（点一下弹窗显示版本/模式/hook/Key 掩码等）、**回复模式**（自动代替聊天 / 手动 @AI 触发）、**API Key**（掩码显示，点输入框才显示明文）、**模型名**（点一下可选 `deepseek-v4-flash` / `deepseek-v4-pro` / 自定义）、**思考延迟（秒）**、**系统提示词**、**聊天风格样本**、**清空记忆**（弹窗确认后清空全部会话上下文）、**恢复默认**。
