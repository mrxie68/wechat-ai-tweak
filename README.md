...
- 单聊、群聊默认都关闭（防误回复）；在设置页打开对应的一键开关，或到某个会话的聊天信息页单独打开“AI 助手”后，就会自动回复**所有消息，不需要任何 @**。
- **没有聊天命令**：聊天里不识别、也不需要任何 `AI xxx / @AI xxx` 命令，你的消息原样收发，一切设置都在设置页完成。
- **每个会话单独开关**：在目标会话（单聊或群聊）的聊天信息页里找到“AI 助手”行拨动开关即可，**永远可用、永远优先**——即使设置页对应的开关是关的，你在某个会话的聊天信息页单独打开它，这个会话照样会回复。
- 设置页的**“单聊 AI 助手”/“群聊 AI 助手”**是**一键批量开关**（改这一类会话的默认状态），不是硬总开关：打开 = 所有没单独改过的会话默认都回复；关掉 = 所有没单独改过的会话默认不回复；单独改过的会话始终以单独设置为准。
