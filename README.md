...
- **链路测试**：让对方（或小号）发 `@AI 测试`，如果收到“插件链路正常”的回复，说明收/发消息 hook 都正常，问题在 API 调用（检查 Key、网络、余额）；如果收不到，说明当前微信版本的消息路径还没适配。
- **状态查询**：发 `@AI 状态`（文件助手即可），会回复插件版本、模式、三个 hook 是否安装成功、API Key 掩码和长度、白名单——排查时把这个回复发给我。
