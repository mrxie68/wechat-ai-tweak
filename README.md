...
- 设置页从上到下：**机器人开关**、**AI 状态**（点一下弹窗显示版本/模式/hook/Key 掩码/风格样本等，完整内容自动复制到剪贴板；弹窗里有"测试对话"按钮，一键验证 Key/模型/参数/Few-Shot/网络全链路）、**回复模式**（自动代替聊天 / 手动 @AI 触发）、**API Key**（掩码显示，点输入框才显示明文）、**模型名**（点一下可选 `deepseek-v4-flash` / `deepseek-v4-pro` / 自定义）、**思考延迟（秒）**、**高级参数**（温度 0~2 默认 0.5、频率惩罚/存在惩罚 0~2 默认 0.4，抑制发散和复读机；模拟打字开关默认开，关闭后回复即时发出）、**系统提示词**、**聊天风格样本**、**基础信息**（填你的真实信息：称呼、城市/农村、工作、家庭、爱吃什么等，AI 聊天只以此为准，没写的绝不编造）、**清空记忆**（弹窗确认后清空全部会话上下文和所有已学习的风格档案）、**恢复默认**。
