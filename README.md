...
- 设置页从上到下：**机器人开关**、**AI 状态**（点一下弹窗显示版本/模式/hook/Key 掩码等，完整内容自动复制到剪贴板）、**回复模式**（自动代替聊天 / 手动 @AI 触发）、**API Key**（掩码显示，点输入框才显示明文）、**模型名**（点一下可选 `deepseek-v4-flash` / `deepseek-v4-pro` / 自定义）、**思考延迟（秒）**、**系统提示词**、**聊天风格样本**、**清空记忆**（弹窗确认后清空全部会话上下文和所有已学习的风格档案）、**恢复默认**。
